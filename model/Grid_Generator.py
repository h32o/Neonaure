...
def generation(row, col, nb_pattern, pourcentage_given):
    """
    Générateur de grille Néonaure.

    Approche "solution-first" en 3 étapes :

    1. Remplir la grille avec des valeurs valides en respectant la contrainte
       de voisinage Moore (backtracking ligne par ligne, très rapide).

    2. Partitionner la grille en motifs connexes orthogonalement.
       Les cellules d'un motif ont toutes des valeurs distinctes par construction,
       ce qui garantit que la contrainte motif est satisfaite.

    3. Masquer un pourcentage de cellules pour créer le puzzle.

    Complexité : quelques millisecondes pour une grille 8x8.
    """
